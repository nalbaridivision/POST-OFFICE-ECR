...
function GroupCard({ groupKey, records, groupLabel }: {
  groupKey:string; records:ECRRecord[]; groupLabel:string;
}) {
  const [open,setOpen] = useState(false);
  ...
  const poor     = records.filter(r=>r.ecr<80).length;
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:6}}>
          {[
            ...
            {label:`${pl>=0?"Surplus":"Deficit"}`,
              val:(pl>=0?"+":"-")+fmtR(pl), color:pl>=0?"#15803D":"#B91C1C"},
...
export default function ReportsPage() {
  ...
  conData.forEach(r=>{
    const key = groupBy==="circle"      ? (r.circleCode||"Unknown")
               : groupBy==="division"   ? (r.divisionCode||"Unknown")
               : (r.subDivCode||"Unknown");
  ...
  async function exportMonthly() {
    ...
    const rows = monthlyData.map((r,i)=>({
      ...
      "POSB Live":  r.heads?.posb_live||0,
      "PLI":        r.heads?.pli_premium||0,
      "RPLI":       r.heads?.rpli_premium||0,
    }));
    ...
    if (!detail) {
      Object.entries(grouped).forEach(([key,recs])=>{
        ...
        rows.push({
          ...
          TotalExpend: exp,
          "Net P&L":   inc-exp,
          "Avg ECR%":  avg.toFixed(2),
          "≥100% ECR": recs.filter(r=>r.ecr>=100).length,
          "<80% ECR":  recs.filter(r=>r.ecr<80).length,
        });
      });
    } else {
      Object.entries(grouped).forEach(([key,recs])=>{
  ...
  async function exportFilterResult() {
    ...
    const paramLabels: Record<FilterParam,string> = {
      ecr:"ECR%", posb:"POSB Live A/C", pli:"PLI Premium", rpli:"RPLI Premium"
    };
    const rows = filterResult.map((r,i)=>({
      ...
      Expenditure: r.expenditure,
      "P&L":       r.income-r.expenditure,
      [paramLabels[filterParam]]: filterParam==="ecr"
        ? r.ecr.toFixed(2)
        : filterParam==="posb"
          ? r.heads?.posb_live||0
          : filterParam==="pli"
            ? r.heads?.pli_premium||0
            : r.heads?.rpli_premium||0,
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    ...
    <div style={{paddingBottom:80,background:"#F0F4F8",minHeight:"100vh",
      ...
      <div style={{padding:"12px 12px 0"}}>
              ...
              <div style={{display:"flex",gap:10,marginTop:10}}>
                <div style={{flex:1}}>
                  ...
                  <select style={inp} value={year1} onChange={e=>setYear1(e.target.value)}>
                    {[curYear-3,curYear-2,curYear-1,curYear].map(y=><option key={y} value={y}>{y}</option>)}
                  </select>
                </div>
                <div style={{flex:1}}>
            ...
            <div style={fCard}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>exportConsolidated(false)}
                  style={{...exportBtn,flex:1,fontSize:11}}>
                  📥 Summary
                </button>
                <button onClick={()=>exportConsolidated(true)}
                  style={{...exportBtn,flex:1,fontSize:11,background:"#0F766E"}}>
                  📥 Detailed
                </button>
              </div>
            </div>
              ...
              : Object.entries(grouped)
                  ...
                  .map(([key,recs])=>(
                    <GroupCard key={key} groupKey={key} records={recs}
                      groupLabel={groupBy==="circle"?"Circle"
                        :groupBy==="division"?"Division":"Sub Division"}/>
                  ))
            }
          </>
        )}
        ...
        {activeTab==="filter" && !loading && (
          <>
            <div style={fCard}>
              <div style={sHead}>Search & Filter Offices</div>
                ...
                {filterResult.length===0
                  ...
                  : (
                    ...
                    <div style={{background:"#fff",borderRadius:12,
                      ...
                      <div style={{display:"grid",
                        ...
                        {["#","Office","ECR","Income","P&L"].map(h=>(
                          <div key={h} style={{fontSize:10,fontWeight:700,
                            ...
                            textAlign:h==="#"||h==="ECR"||h==="Income"||h==="P&L"
                      ...
                      <div style={{display:"grid",
                        gridTemplateColumns:"auto 1fr auto auto auto",
                        gap:0,background:"#EBF8FF",
                        padding:"10px 14px",borderTop:"2px solid #BEE3F8"}}>
                        <div/>
                        <div style={{fontSize:12,fontWeight:700,color:"#1D4ED8"}}>
                          TOTAL ({filterResult.length} offices)
                        </div>
                        <div style={{fontSize:12,fontWeight:700,color:ecrColor(
                          filterResult.reduce((a,r)=>a+r.ecr,0)/filterResult.length
                        ...
                        <div style={{fontSize:12,fontWeight:700,
                          color:filterResult.reduce((a,r)=>a+(r.income-r.expenditure),0)>=0
                            ?"#16A34A":"#DC2626",
                          textAlign:"right" as const}}>
